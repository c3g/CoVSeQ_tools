...
bioinf.metrics <- readr::read_csv("report/report_metrics_ivar.csv")
```
...
This table is also available as a [csv file](report_metrics_ivar.csv) and a [tsv file](report_metrics_ivar.tsv). Additional metrics can be found in the full processing results under `metrics`. 
